...
void main() {
    ...
    // CHECK-NEXT: [[a_as_float:%\d+]] = OpConvertSToF %float [[a]]
    // CHECK-NEXT: OpStore %result [[a_as_float]]
    int a;
    result = asfloat(a);

    // CHECK-NEXT: [[b:%\d+]] = OpLoad %uint %b
    // CHECK-NEXT: [[b_as_float:%\d+]] = OpConvertUToF %float [[b]]
    ...
    // CHECK-NEXT: [[d_as_float:%\d+]] = OpConvertSToF %float [[d]]
    // CHECK-NEXT: OpStore %result [[d_as_float]]
    int1 d;
    result = asfloat(d);

    // CHECK-NEXT: [[e:%\d+]] = OpLoad %uint %e
    // CHECK-NEXT: [[e_as_float:%\d+]] = OpConvertUToF %float [[e]]
    ...
    // CHECK-NEXT: [[g_as_float:%\d+]] = OpConvertSToF %v4float [[g]]
    // CHECK-NEXT: OpStore %result4 [[g_as_float]]
    int4 g;
    result4 = asfloat(g);

    // CHECK-NEXT: [[h:%\d+]] = OpLoad %v4uint %h
    // CHECK-NEXT: [[h_as_float:%\d+]] = OpConvertUToF %v4float [[h]]
