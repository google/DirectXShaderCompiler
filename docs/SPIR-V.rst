...
- `asfloat`: converts the component type of a scalar/vector/matrix from float, uint, or int into float. Uses ``OpConvertSToF`` or ``OpConvertUToF`` where necessary. This method currently does not support taking non-float matrix arguments.

- `asint`: converts the component type of a scalar/vector/matrix from float or uint into int. Uses ``OpConvertFToS`` or ``OpBitcast`` where necessary. This method currently does not support conversion into integer matrices.

- `asuint`: converts the component type of a scalar/vector/matrix from float or int into uint. Uses ``OpConvertFToU`` or ``OpBitcast`` where necessary. This method currently does not support conversion into unsigned integer matrices.
...
+-----------------------------+-----------------------------------------------------+
|   HLSL intrinsic function   |   SPIR-V instruction (in the GLSL instruction set   |
+-----------------------------+-----------------------------------------------------+
